...
class SocketBootstrap {
  constructor (opts) {
    this.opts = opts;
    let dOpts = opts.deviceName.split(':');
    this.address = dOpts[0];
    if (dOpts.length > 1) {
      this.port = dOpts[1];
    } else {
      this.port = 8888;
  ...
  async connectSocket () {
    log.debug(`connectSocket ${this.opts.address}:${this.port}`);
    try {
      return await new Promise((resolve, reject) => {
        try {
          if (!this.socketClient) {
            this.socketClient = net.connect(this.port, this.opts.address);
